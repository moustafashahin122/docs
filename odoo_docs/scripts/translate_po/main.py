...
async def solve(new_file: str, old_file: str, arguments):
    ...
    for line in lines:
        print(f"Translating {line.msgid}")
        if not line.msgstr.strip() or len(line.msgstr.strip()) == 0:
            line.msgstr = polib.unescape(await translate(polib.escape(line.msgid), arguments))
...
async def run(**kwargs):
    ...
    for file in os.listdir(arguments.src):
        if recognize_po_file(file):
            found_files = True
            await solve(os.path.join(arguments.dest, file), os.path.join(arguments.src, file), arguments)
